...
//		getLastUpdatedSeeds();
	...
	public static List<Seed> getLastUpdatedSeeds() {
		...
		String selectSQL = "SELECT * from mailinglocaweb.FEEDER WHERE  SEED='bdchon@yahoo.com' AND FEEDER_UPDATED_DATE BETWEEN '" + formatter.format(oneHourAgo) + "' AND '" + formatter.format(now) + "' ORDER BY FEEDER_UPDATED_DATE DESC LIMIT 20";
//		String selectSQL = "SELECT * from mailinglocaweb.FEEDER WHERE SEED='bdchon@yahoo.com'";
	...
	public static List<Seed> getSeedsWithNoSubscriptions(int index, int top) {
		...
		String selectSQL = "SELECT * from mailinglocaweb.FEEDER WHERE FEEDER.ID >= "+index+" AND FEEDER.ID < "+top+" AND FEEDER.SUBSCRIPTION is NULL ORDER BY FEEDER.ID ASC LIMIT 1";
		List<Seed> seeds = new ArrayList<Seed>();
		try {
			dbConnection = getDBConnection();
			statement = dbConnection.createStatement();
